...
"Left Join " & _
"( " & _
"select tblDebitNote.ContractID,isnull(sum(tblTransQuota.Quantity),0) TQTY " & _
"from tblDailyQuota " & _
"left join  tblTransQuota on tblTransQuota .QuotaID =tblDailyQuota .QuotaID /*and tblDailyQuota.Finished=0*/ " & _
"left join  tblTransactions T on (tblTransQuota.TransID=T.Trans_ID) left join tblDebitNote on tblDailyQuota .DebitNoteID =tblDebitNote .DebitNoteID  where CONVERT(DateTime,T.Out_Date,103) between '" & gMethods.GetShifName_And_SearchDateFT(True, dtpReportQuota.Value.Date, dtpReportQuota.Value.Date)(0) & "' and '" & gMethods.GetShifName_And_SearchDateFT(True, dtpReportQuota.Value.Date, dtpReportQuota.Value.Date)(1) & "'" & _
...
"Left Join " & _
"( " & _
"Select  TT.QuotaID,sum(TT.Quantity) TransQ " & _
"From tblTransQuotaWithOutContract TT " & _
"Left join tblTransactions T on(TT.TransID=T.Trans_ID) " & _
"where Convert(Datetime,Out_Date,103) Between '" & gMethods.GetShifName_And_SearchDateFT(True, dtpReportQuota.Value.Date, dtpReportQuota.Value.Date)(0) & "' and '" & gMethods.GetShifName_And_SearchDateFT(True, dtpReportQuota.Value.Date, dtpReportQuota.Value.Date)(1) & "'" & _
