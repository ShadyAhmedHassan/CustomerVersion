...
#Region "Form"
    Private Sub cfrmLogin_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Dim strMiniIPAddress As String = ""

        Dim LastIndex As Integer
        LastIndex = strIPAddress.LastIndexOf(".")
        strMiniIPAddress = strIPAddress.Substring(0, LastIndex)
        Dim ss As Integer
        If gAdo.CmdExecScalar("select ISNULL(Min(Scale_ID),0) from tblScale where Scale_IsDeleted='false' and Scale_IP = '" & strIPAddress & "'") > 0 Then
            gAdo.CtrlItemsLoad("Scale_ID", "Scale_Name", "tblScale", cboScale, "", "Scale_IsDeleted='false' and Scale_IP Like '" & strIPAddress & "' ORDER BY Scale_ID")
        Else
            If (gAdo.CmdExecScalar("select Min(Scale_ID) from tblScale where Scale_IsDeleted='false' and Scale_IP like  '%" & strMiniIPAddress & "%'") > 0) Then
                gAdo.CtrlItemsLoad("Scale_ID", "Scale_Name", "tblScale", cboScale, "", "Scale_IsDeleted='false' and Scale_IP Like '%" & strMiniIPAddress & "%' ORDER BY Scale_ID")
            End If
        End If
...
#Region "Login"
    ...
    Private ReadOnly Property IsPasswordValid() As Boolean
        Get
            ...
            UserCount = gAdo.CmdExecScalar("Select count (User_ID) from TblUser WHERE User_IsDeleted=0")
            ...
            If gLogin.F_Login(cboEmpName.Text, Trim(txtPassword.Text)) = Trim(txtPassword.Text) Then
                ...
                gUserID = gAdo.CmdExecScalar("select User_ID from TblUser where User_NickName='" & cboEmpName.Text & "' AND User_IsDeleted=0")
...
Ret:
    ...
    Private Sub btnLogin_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnLogin.Click
        If cboScale.SelectedValue Is Nothing Then
            MsgBox("من فضلك إختر ميزان")
            Exit Sub
        End If
        Dim UID As String = gAdo.CmdExecScalar("select User_ID from tblUser where User_NickName='" & cboEmpName.Text.Trim() & "'")
        cboEmpName.SelectedValue = UID
        

        If Not IsPasswordValid Then Exit Sub
        cfrmMain.ScaleNo = cboScale.SelectedValue
        ...
        gUserO.ID = cboEmpName.SelectedValue
        gUserO.Refresh()
        gUserFullName = gUserO.Name
        gUserPic = PicBoxUser.Image
        gLoginTime = Now


        '' Elsamn 15/1/2020''
        '' code to get scale in cfrmReports
       
        cfrmReports.ScaleNo = cboScale.SelectedValue
...
#End Region
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        gAdo.CtrlItemsLoad("Scale_ID", "Scale_Name", "tblScale", cboScale, "", "scale_IsDeleted='false' ORDER BY Scale_ID")
    End Sub
End Class